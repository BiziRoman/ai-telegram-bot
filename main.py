...
Заголовок: Мужские кроссовки с дышащей сеткой и амортизацией
Преимущества:
• Сетка AirMesh — ноги дышат даже в жару, нет неприятного запаха
• Подошва EVA — амортизирует каждый шаг, ноги не устают к вечеру
• Нескользящая резиновая подошва — безопасно на мокром асфальте и льду
...
async def generate_description(message: types.Message):
    ...
    if get_user_stats(user_id) >= 3:
        await message.answer(
            ...
            "📩 Для подключения напишите: @BiziRoman",
            parse_mode="Markdown"
        )
        return
    ...
    try:
        # НОВАЯ МОДЕЛЬ: NVIDIA Nemotron (лучшее качество для русского языка)
